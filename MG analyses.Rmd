...
```{r}
#Data organization
mg.spring2017.6month <- filter(mg, mg$planting.date == "Spring" & mg$sample.point == "6month")
...
```{r}
#Data management- excluding plot with no discs remaining
mg.spring2017.6month.den <- subset(mg.spring2017.6month, no.discs > 0)
...
```{r}
#Data organization
mg.fall2017.6month <- filter(mg, mg$planting.date == "Fall" & mg$sample.point == "6month")
...
```{r}
#Data management- excluding plot with no discs remaining
mg.fall2017.6month.den <- subset(mg.fall2017.6month, no.discs > 0)
...
bargraph.CI(x.factor = identity, response = shoots.per.disc, data = mg.fall2017.6month.den, ylim = c(0,15), ylab = "shoots per disc", xlab = "site identity")
...
bargraph.CI(x.factor = no.sites, response = shoots.per.disc, data = mg.fall2017.6month.den.Poly5, ylim = c(0,15), ylab = "shoots per disc", xlab = "no sites per disc")
...
plot(shoots.per.disc ~ identity, data = mg.fall2017.6month.den.Poly3, ylim = c(0,25))
plot(shoots.per.disc ~ no.sites, data = mg.fall2017.6month.den.Poly3, ylim = c(0,25))
```
...
plot(canopy.height ~ transect, data = mg.fall2017.6month.ch)
plot(canopy.height ~ identity, data = mg.fall2017.6month.ch)
...
bargraph.CI(x.factor = transect, response = no.discs, data = mg.fall2017.6month, ylim = c(0,3), ylab = "No discs per plot")
```
...
mg.plot = mg %>%
  ...
  summarize(shoot.per.plot = mean(no.shoots, na.rm = T), discs.per.plot = mean(no.discs, na.rm = T))
