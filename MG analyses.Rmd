...
#Site-Survey Wasting Disease Analyses
...
##Data management
```{r}
##Wasting disease analyses
sc.wd <-sc

sc.wd.leaf.prop <- sc.wd %>%
  filter(below.sheath == 0) %>%
  group_by(year, day.collected, month.collected, site, shoot.id) %>%
...
##Wasting disease analyses
###proportion infected
p <- ggplot(filter(sc.wd.leaf.prop, lesion.present != 0), aes(x = month.collected, y = lesion.prop, fill = site)) + 
  ...
  labs(x = "Month", y = "Proportion of leaves infected") +
  theme_classic()
p
```
#*******************************
