...
plot(shoots.per.disc ~ diversity.treatment, data = mg.spring2017.6month.den)
plot(shoots.per.disc ~ identity, data = mg.spring2017.6month.den)
...
anova(model.5poly) #no.discs, no.sites, identity --> significant; mono < poly
plot(shoots.per.disc ~ no.sites, data = mg.spring2017.6month.den.Poly5)
...
bargraph.CI(x.factor = planting.date, response = no.discs, group = identity, data = mg, legend = T, x.leg = 7, y.leg =6, ylim = c(0,6)) #spring poly5s had fewer discs remaining after 1 month
...
```{r}
mg.plot = mg %>%
  group_by(planting.date, identity) %>%
  summarize(shoot.per.plot = mean(no.shoots, na.rm = T))
